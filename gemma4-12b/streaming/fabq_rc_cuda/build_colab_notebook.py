...
print(f"Building for sm_{arch} only (full output, not -q)")
!TORCH_CUDA_ARCH_LIST="{arch}" pip install -e . --no-deps 2>&1 | tail -40
""")

code("""# Cell 6: verify the extension is importable BEFORE pytest
import glob, importlib, sys

# Find the built .so
candidates = (
    glob.glob('fabq_rc_cuda/_C*.so')
    + glob.glob('_C*.so')
    + glob.glob('build/**/_C*.so', recursive=True)
)
print("found .so files:", candidates)
assert candidates, (
    "no _C*.so found - the build in cell 5 failed silently. "
    "Re-run cell 5 without the | tail to see the actual error."
)

# Try the import the test will do
sys.path.insert(0, '.')
try:
    import fabq_rc_cuda
    import fabq_rc_cuda._C
    print("import OK; V2_AVAILABLE =", getattr(fabq_rc_cuda._C, "V2_AVAILABLE", False))
except Exception as e:
    print(f"import FAILED: {type(e).__name__}: {e}")
    raise
""")

code("""# Cell 7: parity tests
!python -m pytest tests/test_v2_kernel.py -v 2>&1 | tail -80
""")

md("""## Bench: v2 (W4A16 fp16-TC) vs v1 (scalar)

Cell 8 hits the decode path (B*T=1, v2_int4_kernel vectorized scalar wins).
Cell 9 hits the batched path (B*T=16, v2_int4_via_fp16_tc_kernel should kick in).
""")

code("""# Cell 8: bench - decode path (B*T=1)
import torch, time
...
code("""# Cell 9: bench - batched decode (B*T=16, TC path's target regime)
B, T = 1, 16
...
# Also write a .py version with # %% cell markers, for VSCode / nbconvert users.
PY_OUT = Path(__file__).parent / "colab_test.py"
py_lines = ['"""FABQ-RC v2 kernel test (Colab cells, .py form).',
            '',
            'Regenerated by build_colab_notebook.py - edit that script to change cells.',
            '"""',
            '']
for c in cells:
    if c['cell_type'] == 'markdown':
        ...
        py_lines.append('"""')
        py_lines.extend(line.rstrip() for line in c['source'])
        py_lines.append('"""')
    else:
        py_lines.append('# %%')
